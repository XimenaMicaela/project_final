...
function createUser(req, res) {
  // Extraemos toda la informacion del cuerpo de la solicitud (postman).
...
function deleteUser(req,res){
  ...
  const userId = req.params.id;
    ...
    .then(()=> res.status(200).send("Usuario eliminado correctamente")) // Enviamos una confirmacion al cliente de que el usuario se elimino correctamente
    .catch((err) => {
        console.error(err);
